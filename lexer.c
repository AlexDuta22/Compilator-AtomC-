...
// Adauga un token la lista de tokeni
...
{
	//returneaza un string cu caracterele de la begin la end
	size_t length = end - begin;
	char *string = (char *)safeAlloc(length + 1); //alocam memorie pentru string
	strncpy(string, begin, length); //copiem caracterele de la begin la end
	string[length] = '\0'; //adaugam terminatorul de sir
	return string;
}

//Functia principala de tokenizare
Token *tokenize(const char *pch)
{
	const char *start;
	Token *tk;

    	//adaugam fiecare token	
	for(;;)
	{
		switch(*pch)
		{
			...
			default:
				...
				else if (isdigit(*pch))
				{
					...
					if (*pch == '.')
					{
						pch++;
						if(!isdigit(*pch))
							err("Invalid double \'.\' should be a digit:"); 
						while (isdigit(*pch))
							pch++;
						if (*pch == 'e' || *pch == 'E')
						{
					...
					else if (*pch == 'e' || *pch == 'E')
					{
						...
						if(!isdigit(*pch))
							err("Missing a digit to be a DOUBLE");
						if (isdigit(*pch))
						{
							while (isdigit(*pch))
								pch++;
